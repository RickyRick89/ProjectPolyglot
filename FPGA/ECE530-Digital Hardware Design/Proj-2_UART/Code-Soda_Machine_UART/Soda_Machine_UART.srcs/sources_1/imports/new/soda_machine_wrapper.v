...
module soda_machine_wrapper(led,TxD,pb,sw,clk);
    ...
    reg [25:0] wait_cycles;        // Count for the number of cycles to wait
    reg [25:0] wait_count;         // Counter for wait cycles
    ...
    reg [4:0] txt_b;
	reg [7:0] chng;
	(* mark_debug = "true" *) reg TxFIFO_wr;
	(* mark_debug = "true" *) reg [9:0] TxDin;
	...
	wire w_clk_50mhz;
	(* mark_debug = "true" *) wire w_TxFIFO_full;
	(* mark_debug = "true" *) wire w_TxFIFO_empty;


   /*--------- Instantiations ---------*/
    clk_wiz_0 clk2 (.clk_out1(w_clk_50mhz),.resetn(w_n_rst),.clk_in1(clk));

	edge_det_and_synch pb1_edge_to_pulse (.Rise(w_pb1_rise), .Fall(w_pb1_fall), .Clock_in(w_clk_50mhz), .n_rst(w_n_rst), .Sig(pb[1])); // Edge detector for push button 1
	edge_det_and_synch pb2_edge_to_pulse (.Rise(w_pb2_rise), .Fall(w_pb2_fall), .Clock_in(w_clk_50mhz), .n_rst(w_n_rst), .Sig(pb[2])); // Edge detector for push button 2
	edge_det_and_synch pb3_edge_to_pulse (.Rise(w_pb3_rise), .Fall(w_pb3_fall), .Clock_in(w_clk_50mhz), .n_rst(w_n_rst), .Sig(pb[3])); // Edge detector for push button 3

	soda_machine_top sm (.d(w_d),.tot(w_tot),.chng(w_chng),.s(price_acptd),.a(w_a),.c(w_pb2_rise),.clkin(w_clk_50mhz),.n_rst(w_n_rst)); // Top level module for the soda machine
	...
	UART_top uart1 (.TxD(TxD),.TxFIFO_full(w_TxFIFO_full),.TxFIFO_empty(w_TxFIFO_empty),.clk_50mhz(w_clk_50mhz),.TxDin(TxDin),.TxFIFO_wr(TxFIFO_wr),.config_reg(w_config_reg),.clk(clk),.n_rst(w_n_rst));   
	...
	begin
		...
		begin
			...
			if (((characters_r1 != prev_characters_r1)|(characters_r2 != prev_characters_r2)|(characters_r3 != prev_characters_r3)) & !characters_valid)
			begin
			...
			begin
				...
				else if (w_TxFIFO_empty)
				begin
					if (char_num == 96)
					begin
						if (line_num == 2)
						begin
							TxDin <= 10'd4; //End of Transmission
							TxFIFO_wr <= 1'b1;
					...
					begin
						case (line_num)
							0:
							begin
								TxDin[0] <= prev_characters_r1[char_num];
								TxDin[1] <= prev_characters_r1[char_num+1];
								TxDin[2] <= prev_characters_r1[char_num+2];
								TxDin[3] <= prev_characters_r1[char_num+3];
								TxDin[4] <= prev_characters_r1[char_num+4];
								TxDin[5] <= prev_characters_r1[char_num+5];
								TxDin[6] <= prev_characters_r1[char_num+6];
								TxDin[7] <= prev_characters_r1[char_num+7];
								TxDin[8] <= 1'b0;
								TxDin[9] <= 1'b0;
								TxFIFO_wr <= 1'b1;
								char_num <= char_num + 8;
							...
							1:
							begin
								TxDin[0] <= prev_characters_r2[char_num];
								TxDin[1] <= prev_characters_r2[char_num+1];
								TxDin[2] <= prev_characters_r2[char_num+2];
								TxDin[3] <= prev_characters_r2[char_num+3];
								TxDin[4] <= prev_characters_r2[char_num+4];
								TxDin[5] <= prev_characters_r2[char_num+5];
								TxDin[6] <= prev_characters_r2[char_num+6];
								TxDin[7] <= prev_characters_r2[char_num+7];
								TxDin[8] <= 1'b0;
								TxDin[9] <= 1'b0;
								TxFIFO_wr <= 1'b1;
								char_num <= char_num + 8;
							...
							2:
							begin
								TxDin[0] <= prev_characters_r3[char_num];
								TxDin[1] <= prev_characters_r3[char_num+1];
								TxDin[2] <= prev_characters_r3[char_num+2];
								TxDin[3] <= prev_characters_r3[char_num+3];
								TxDin[4] <= prev_characters_r3[char_num+4];
								TxDin[5] <= prev_characters_r3[char_num+5];
								TxDin[6] <= prev_characters_r3[char_num+6];
								TxDin[7] <= prev_characters_r3[char_num+7];
								TxDin[8] <= 1'b0;
								TxDin[9] <= 1'b0;
								TxFIFO_wr <= 1'b1;
								char_num <= char_num + 8;
		...
		begin
			...
			case (state)
				...
				begin
                    ...
                    txt_b <= 5'd31;
					chng <= 8'd0;
					wait_cycles <= 26'd50000000;     // Initialization of cycles to wait
					wait_count <= 26'd0;          // Initialization of counter
