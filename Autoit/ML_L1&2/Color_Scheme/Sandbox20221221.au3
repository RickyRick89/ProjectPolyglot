...
For $sFile In $aFiles
	...
	$sDisplayName = StringLeft(StringReplace(StringTrimLeft($sFile, StringInStr($sFile, ' ')), '.xml', ''), 31)
	...
	For $sLine In $aFileLines
		...
		EndIf
		$vReturn = _LineSearch($sLine, '</' & $aItemName[UBound($aItemName) - 1])
		If $vReturn <> -1 Then
			_ArrayDelete($aItemName, UBound($aItemName) - 1)
		EndIf

		$vReturn = _LineSearch($sLine, '</group>')
		If $vReturn <> -1 Then
		EndIf
