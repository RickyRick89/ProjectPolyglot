...
$sWorkbookPath = "C:\Users\rgrov\OneDrive - Bit-Wise Automation LLC\Richards_Docs\My_Scripts\PID_CHECK\My_M&I.xlsx"
...
For $iInc = 245 To UBound($aSheet) - 1
;~ 	$sType = 'EC'
	$sType = $aSheet[$iInc][3]
	$sTag = $aSheet[$iInc][5]
	$sStatus = $aSheet[$iInc][21]
	$sDescription = $aSheet[$iInc][6]
	$sSearch = $sType & ' ' & $sTag
	...
	If $bFound Then
		...
		ElseIf $iResponse = $IDNO Then
			$oWorkbook.Sheets($sSheetName).Range('G' & $iInc + 1).Value = InputBox('Description', 'Description', $sDescription)
		EndIf
	Else
