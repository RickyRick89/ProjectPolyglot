...
For $x = 1 To $aTagFileNames[0]
	...
	For $iRow = 1 To UBound($aNewSheet) - 1
		$sInterlock = $aNewSheet[$iRow][0]
		$sDesc = $aNewSheet[$iRow][1]
		$aTemp = StringSplit($sInterlock, '.')
		$sRootTag = $aTemp[1]
		$iIntlkNum = Number(StringRight($sInterlock, 2))
		ConsoleWrite(@CRLF & '++++>' & $sInterlock)
	...
	_Excel_RangeWrite($oWorkbook, 'P_Intlk', $aConfigSheet, 'E10')
