	...
	For $y = 1 To $aL5XFileNames[0]
		...
		If StringInStr($aL5XFileNames[$y], $sControllerName) Then
			...
			For $z = 1 To $aL5XFileLinesTemp[0] - 2
				If StringInStr($aL5XFileLinesTemp[$z], 'Perm.Inp_') Then
...
;~ 			_Excel_BookClose($oWorkbook)
	...
	For $iRow = 7 To UBound($aSheet) - 1
		...
		If Not StringInStr($sDataType, 'P_Perm') Then
			ContinueLoop
		EndIf
		...
		For $iPermNum = 0 To 8
			...
			$sPermissiveTag = $sTagName & '.Inp_Perm0' & $iPermNum
			ConsoleWrite(@CRLF & '>>>>>' & $sPermissiveTag)
			$sPermissiveDesc = $aConfigSheet[$iConfigRow][($iPermNum * 5) + 7]
			ConsoleWrite(@CRLF & '>>>' & $sPermissiveDesc)

			$iL5XRow = _ArraySearch($aL5XFileLines, 'OTE(' & $sPermissiveTag & ')', 0, 0, 0, 1)
			If $iL5XRow >= 0 Then
				$aBranches = StringSplit($aL5XFileLines[$iL5XRow], ',')
				$iTempRow = _ArraySearch($aBranches, 'OTE(' & $sTagName & '.Inp_Perm0' & $iPermNum & ')', 0, 0, 0, 1)
			...
			Local $aTemp[1][3] = [[$sPermissiveTag, $sPermissiveDesc, $sTempLogic]]
