	...
	For $y = 1 To $aL5XFileNames[0]
		...
		If StringInStr($aL5XFileNames[$y], $sControllerName) Then
			...
			For $z = 1 To $aL5XFileLinesTemp[0] - 2
				If StringInStr($aL5XFileLinesTemp[$z], '_Interlock.Inp_') Then
...
;~ 					ConsoleWrite(@CRLF & '-->' & $aL5XFileLines[0])
	...
	For $y = 1 To $aConfigFileNames[0]
		...
		If StringInStr($aConfigFileNames[$y], $sControllerName) Then
			...
			$aConfigSheet = _Excel_RangeRead($oWorkbook, 'I_Multiplex_04')
...
;~ 			_Excel_BookClose($oWorkbook)
	...
	For $iRow = 7 To UBound($aSheet) - 1
		...
		$iConfigRow = _ArraySearch($aConfigSheet, $sRootTag & '_Interlock', 0, 0, 0, 1, 1, 2)


		For $iIntlkNum = 0 To 4
			$sTempLogic = ''
			If $iConfigRow < 0 Then ExitLoop

			$sInterlockTag = $sRootTag & '_Interlock.Inp_0' & $iIntlkNum
			ConsoleWrite(@CRLF & '>>>>>' & $sInterlockTag)
			$sInterlockDesc = $aConfigSheet[$iConfigRow][$iIntlkNum + 5]
			...
			If $iL5XRow >= 0 Then
				...
				$iTempRow = _ArraySearch($aBranches, 'OTE(' & $sRootTag & '_Interlock.Inp_0' & $iIntlkNum & ')', 0, 0, 0, 1)
