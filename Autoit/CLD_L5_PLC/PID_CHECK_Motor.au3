...
$sWorkbookPath = "C:\Users\rgrov\OneDrive - Bit-Wise Automation LLC\Richards_Docs\My_Scripts\PID_CHECK\Motor And Instrument List Rev 0.5.xlsx"
...
;~ 	$sType = $aSheet[$iInc][2]
	...
	If $bFound Then
		$oWorkbook.Sheets($sSheetName).Range('D' & $iInc + 1).Style = "Good"

		$iResponse = MsgBox($MB_YESNO, $sSearch, $sControlType & '?')

		If $iResponse = $IDYES Then
			$oWorkbook.Sheets($sSheetName).Range('W' & $iInc + 1).Style = "Good"
		ElseIf $iResponse = $IDNO Then
			$oWorkbook.Sheets($sSheetName).Range('W' & $iInc + 1).Style = "Bad"
		EndIf
	Else
		$oWorkbook.Sheets($sSheetName).Range('D' & $iInc + 1).Style = "Bad"
	EndIf
