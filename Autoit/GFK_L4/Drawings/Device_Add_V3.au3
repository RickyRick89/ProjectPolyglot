...
;~ 	If $Device_Type <> 'ASI_Valve' Then ContinueLoop
	...
	$sDevTemp = FileRead($sUseFilesRoot & $Device_Type & '.dxf')
	...
	_FindReplace($sDevTemp, $aFinds, $aReplaces)

	$sNewTempPath = StringReplace($sTempPath, "Temp.scr", "Temp" & @MIN & @SEC & @MSEC & ".dxf")
	_FileCreate($sNewTempPath)
	FileWrite($sNewTempPath, $sDevTemp)


	$sUseFile = $sNewTempPath
...
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ADD IO ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	If StringInStr($INST_NAME, '-JB') Then
		...
		For $iCol = 1 To 16
			...
			$sDevTemp = FileRead($sUseFilesRoot & $sInstType & '.dxf')
			Local $aFinds[8] = ['[INST_NAME]', '[DESCRIPTION]', '[JB_NAME]', '[NODE_NUM]', '[INPUT_NUM]', '[OUTPUT_NUM]', '[ASM_NAME]', '[TRUNK_NUM]']
			Local $aReplaces[8] = [$INST_NAME, $DESCRIPTION, $JB_NAME, $NODE_NUM, $INPUT_NUM, $OUTPUT_NUM, $ASM_NAME, $TRUNK_NUM + 1]
...
Next
$sNewTempPath = StringReplace($sTempPath, "Temp.scr", "Temp" & @MIN & @SEC & @MSEC & ".scr")
_FileCreate($sNewTempPath)
...
Func _FindReplace(ByRef $sString, $sFind, $sReplace)
	ConsoleWrite("_FindReplace()" & @CRLF)

	If IsArray($sFind) And IsArray($sReplace) Then
		For $iRow = 0 To UBound($sFind) - 1
			$sString = StringReplace($sString, $sFind[$iRow], $sReplace[$iRow])
		Next
	Else
		$sString = StringReplace($sString, $sFind, $sReplace)
	EndIf
