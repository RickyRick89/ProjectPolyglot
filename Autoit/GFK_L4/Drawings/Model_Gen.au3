...
Global $sTempPath = @ScriptDir & "\Templates\Temp.scr"
...
Run('"C:\Program Files\Autodesk\AutoCAD 2024\acad.exe" "'&@ScriptDir & '\Sandbox.dwg" /b "' & $sFirstScriptPath & '"')
...
;~ 		While WinExists('AutoCAD 2024')
;~ 			WinKill('AutoCAD 2024')
;~ 			Sleep(2000)
;~ 		WEnd
;~ 		Exit

;~ 		Run('"C:\Program Files\Autodesk\AutoCAD 2024\acad.exe" "' & $sNewFilePath & '"')
;~ 		Sleep(15000)
		...
		$sNewText = 'text ' & (($iSheetXInc * ($iMasterInc - 1)) + ($iSheetXInc * .15)) & ',3 2 0 ' & $sPanel & @CRLF
		$sNewText &= 'qsave' & @CRLF & 'close' & @CRLF
		_FileCreate($sTempPath)
		FileWrite($sTempPath, $sNewText)
		_RunScript($sTempPath)
