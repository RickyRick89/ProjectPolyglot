...
Global $sLastSelected = 'L04PRC-R1_JB01'
...
Global $aTypes[2][6] = [['AI', 'AO', 'RLY', 'DI', 'DO', 'DI/DO'], ['VBA-4E-KE5-IL', 'VBA-4A-KE5-IJL_UJL', 'VBA-4E4A-KE5-ZEJQ_R', 'VBA-4E4A-KE5-ZEJQ_E2L/VBA-4E4A-KE5-ZEJQ_R', 'VBA-4E4A-KE5-ZEJQ_E2L', 'NODE']]
Global $aIO[1][(UBound($aPanels) - 1) * $iMaxNumOfModules]
Global $aNodes[32][6][2][2] ; Node, Master, Trunk, Extension
$aNodes[0][0][0][0] = "L04PRC-R1-ASM01"
$aNodes[0][1][0][0] = "L04PRC-R2-ASM01"
$aNodes[0][2][0][0] = "L04PRC-R3-ASM01"
$aNodes[0][3][0][0] = "L04PRC-R4-ASM01"
$aNodes[0][4][0][0] = "L04PRC-R5-ASM01"
$aNodes[0][5][0][0] = "L04PRC-R6-ASM01"
	...
	For $iRow = 1 To UBound($aSheet) - 1
		...
		If $aTypes[0][$iTypeIndex] = 'DI/DO' Then
			...
			For $iMaster = 0 To UBound($aNodes, 2) - 1
				...
				For $iTrunk = UBound($aNodes, 3) - 1 To 0 Step -1
					If $bAssigned Then ExitLoop
					...
					If StringRight($sSigType, 1) = 'I' Then
						For $iInput = 1 To 4
							...
							If $iInput = 4 And Not $bAssigned Then
								If $aTypes[1][$iTypeIndex] = 'VBA-4E4A-KE5-ZEJQ_E2L/VBA-4E4A-KE5-ZEJQ_R' Then
									_AddIOMod($iFirstAvailCol, 'VBA-4E4A-KE5-ZEJQ_E2L', $sWiredTo, $sSigType, $sASiMaster)
								Else
...
;~~~~~~~~~~~~~~~~~~~~~~~ Control Valve Feed Back ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
			...
			If Not $bAssigned And $iFirstAvailCol >= 0 Then
				...
				If $aTypes[1][$iTypeIndex] = 'VBA-4E4A-KE5-ZEJQ_E2L/VBA-4E4A-KE5-ZEJQ_R' And StringRight($sSigType, 1) = 'I' Then
					_AddIOMod($iFirstAvailCol, 'VBA-4E4A-KE5-ZEJQ_E2L', $sWiredTo, $sSigType, $sASiMaster)
				Else
					_AddIOMod($iFirstAvailCol, $aTypes[1][$iTypeIndex], $sWiredTo, $sSigType, $sASiMaster)
				EndIf
...
For $iCol = 0 To UBound($aIO, 2) - 1
	...
	$sJB = StringLeft($aIO[0][$iCol], StringInStr($aIO[0][$iCol], '-AM') - 1)
	...
	If $aIO[9][$iCol] = 'VBA-4E-KE5-IL' Or $aIO[9][$iCol] = 'VBA-4A-KE5-IJL_UJL' Then
		$sExcelColumn = 'P'
	Else
		$sExcelColumn = 'H'
	EndIf
...
Func _AddIOMod($iFirstAvailCol, $sModule, $sWiredTo, $sSigType, $sASiMaster)
	ConsoleWrite('_AddIOMod(' & $iFirstAvailCol & ',' & $sModule & ',' & $sWiredTo & ',' & $sSigType & ',' & $sASiMaster & ')' & @CRLF)
	$bAssigned = False
	$aIO[9][$iFirstAvailCol] = $sModule
	$sModuleName = $aIO[0][$iFirstAvailCol]
	ConsoleWrite('->Module Name = ' & $sModuleName & @CRLF)

	For $iMaster = 0 To UBound($aNodes, 2) - 1
		If $bAssigned Then ExitLoop
		$sRemotePanel = StringLeft($sWiredTo, StringInStr($sWiredTo, '-JB') - 1)
		If $aNodes[0][$iMaster][0][0] <> $sASiMaster Then ContinueLoop
		...
		For $iTrunk = 0 To UBound($aNodes, 3) - 1
			...
			For $iNode = 1 To UBound($aNodes, 1) - 1
				If $bAssigned Then ExitLoop
				If $sSigType = 'AI' And $iNode >= 16 Then ContinueLoop
				If $sSigType = 'AO' And $iNode < 16 Then ContinueLoop
				...
				For $iExtension = 0 To UBound($aNodes, 4) - 1
					...
					If $aNodes[$iNode][$iMaster][$iTrunk][$iExtension] = '' Then
						...
						$aNodes[$iNode][$iMaster][$iTrunk][$iExtension] = $sModuleName
