...
$sTagPathRoot = 'C:\Users\rgrov\OneDrive - Bit-Wise Automation LLC\Richards_Docs\My_Scripts\GFK_L4\Audit\Tags'
$sL5XPathRoot = 'C:\Users\rgrov\OneDrive - Bit-Wise Automation LLC\Richards_Docs\My_Scripts\GFK_L4\Audit\L5X'
$sConfigRootDir = 'C:\Users\rgrov\OneDrive - Bit-Wise Automation LLC\Richards_Docs\My_Scripts\GFK_L4\Audit\ConfigTools'
...
For $x = 1 To $aTagFileNames[0]
	...
	$sControllerName = StringLeft($aTagFileNames[$x], StringInStr($aTagFileNames[$x], '_', 0, 2) - 1)
	ConsoleWrite(@CRLF & '==================>' & $sControllerName)
	$sNewFilePath = 'C:\Users\rgrov\OneDrive - Bit-Wise Automation LLC\Richards_Docs\My_Scripts\GFK_L4\Audit\Report\' & $sControllerName & '.csv'
...
;~ 			_Excel_BookClose($oWorkbook)
	...
	For $iRow = 7 To UBound($aSheet) - 1
		...
		If StringInStr($sDataType, 'P_Intlk') Then
			ContinueLoop
		EndIf

		$sRootTag = StringReplace($sTagName, '_Intlk', '')

		ConsoleWrite(@CRLF & '++++++>' & $sRootTag)

		$iConfigRow = _ArraySearch($aConfigSheet, $sRootTag & '_Intlk', 0, 0, 0, 1, 1, 2)
		...
		For $iIntlkNum = 0 To 8
			...
			$iConfigDescCol = _ArraySearch($aConfigSheet, 'CondTxt[' & $iIntlkNum & ']', 4, 0, 0, 0, 1, 7, True)

			$sIntlkTag = $sRootTag & '_Intlk.Inp_Intlk0' & $iIntlkNum
			ConsoleWrite(@CRLF & '>>>>>' & $sIntlkTag)
			$sIntlkDesc = $aConfigSheet[$iConfigRow][$iConfigDescCol]
			ConsoleWrite(@CRLF & '>>>' & $sIntlkDesc)

			$iL5XRow = _ArraySearch($aL5XFileLines, 'OTE(' & $sIntlkTag & ')', 0, 0, 0, 1)
			If $iL5XRow >= 0 Then
				$aBranches = StringSplit($aL5XFileLines[$iL5XRow], ',')
				$iTempRow = _ArraySearch($aBranches, 'OTE(' & $sRootTag & '_Intlk.Inp_Intlk0' & $iIntlkNum & ')', 0, 0, 0, 1)
			...
			Local $aTemp[1][3] = [[$sIntlkTag, $sIntlkDesc, $sTempLogic]]
