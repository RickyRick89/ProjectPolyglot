...
For $iInc = 1 To UBound($aSheet) - 1
	...
	Else
		...
		$sSearch = $sType & ' ' & $sTag
		$sSupplyer = $aSheet[$iInc][10]
	EndIf

	If $sStatus = 'Deleted' Then ContinueLoop
	If $sSupplyer <> '' Then ContinueLoop
...
;~ 	If String(Number($sTag)) <> $sTag Then ContinueLoop
	...
	$sSelectedSupply = SupplyPopup()

	If StringInStr($sSheetName, 'Motor') Then
		$oWorkbook.Sheets($sSheetName).Range('J' & $iInc + 1).Value = $sSelectedSupply
	Else
		$oWorkbook.Sheets($sSheetName).Range('K' & $iInc + 1).Value = $sSelectedSupply
	EndIf
...
Func SupplyPopup()
	...
	GUICtrlSetData($combo, "RJX|Others|OEM|?", $sLastSelected) ; Set the options and default value
