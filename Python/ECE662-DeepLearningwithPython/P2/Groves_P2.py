...
# ===================================================================================================
class CustomFER2013(VisionDataset):
    """`FER2013
    ...
    with torch.no_grad():
        for images, labels in dataloader:
            ...
            for img, label, pred in zip(images, labels, predictions):
                img_unnormalized = unnormalize(img.clone(), mean, std)

                # Collect correct examples for each category
                if len(correct_examples[label]) < 1 and pred == label:
                    correct_examples[label].append(img_unnormalized.cpu())
                ...
                if pred != label and incorrect_examples[label] is None:
                    if len(correct_examples[pred]) >= 1:
                        incorrect_examples[label] = (correct_examples[pred][0], img_unnormalized.cpu(), label.item(), pred.item())
...
if os.path.exists(model_file_path ):
    ...
    model.load_state_dict(torch.load(model_file_path, map_location=torch.device(map_location)))
...
visualize_incorrect_examples(model, test_loader, labels_dict,n_categories=len(labels_dict))
