...
class Net(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(1, 10, stride=1, kernel_size=3, padding=1, padding_mode='replicate')  # makes 20 maps of 48x48
        self.pool1 = nn.MaxPool2d(2, 2)                              # 20 maps of 24x24
        self.conv2 = nn.Conv2d(10, 20, stride=1, kernel_size=3, padding=1, padding_mode='replicate') # 40 maps of 24x24
        self.pool2 = nn.MaxPool2d(2, 2)                              # 20 maps of 12x12
        self.conv3 = nn.Conv2d(20, 40, stride=1, kernel_size=3, padding=1, padding_mode='replicate') # 40 maps of 24x24
        self.pool3 = nn.MaxPool2d(2, 2)                              # 20 maps of 6x6
        self.fc1 = nn.Linear(40*6*6, 300)                       # FC 40x6x6 to 300 neurons                   
        self.fc2 = nn.Linear(300, 150)                              # FC 300 to 150 neurons
        self.fc3 = nn.Linear(150,7)                  # 150 to 7 neurons to form logits
    def forward(self, x):
...
# ===================================================================================================
model_dir = f'./models/{epochs}_{learning_rate}_{BATCH_SIZE}-w_scheduler{now}'
...
if os.path.exists(model_file_path ):
    ...
    plt.ylabel("Accuracy")
    plt.legend()
    plt.savefig(f"FER_validation_accuracy-e{epochs}-lr{learning_rate}-bs{BATCH_SIZE}-w_scheduler{now}.png")
    plt.show()
    ...
    plt.legend()
    plt.savefig(f"FER_validation_loss-e{epochs}-lr{learning_rate}-bs{BATCH_SIZE}-w_scheduler{now}.png")
    plt.show()
