...
int main(){
	...
	while(1){
	    if (timeoutCounter > 0){
	        ...
	        resp_rdy = 0;                   // Reset the response ready flag.
	    } else if (timeoutCounter == 0 && bufferIndex > 0) { // Check if timeout has occurred and there is data in the buffer.
	        if (!resp_rdy){
	            rec_cnt = bufferIndex;      // Record the count of received bytes.
	            resp_rdy = 1;               // Set the response ready flag.
	        }

	        // Calculate the end of the current EEPROM page and how many bytes to write.
	        uint8_t currentPageEnd = PAGE_SIZE - (PTR % PAGE_SIZE);
	        uint8_t bytesToWrite = (bufferIndex <= currentPageEnd) ? bufferIndex : currentPageEnd;

	        // Handle odd byte count at the end of a page by reading the first byte of the next page.
	        //		This is necessary because I have to use 16bit data size. (When data size is set to 8bit, there are still 16 sclk cycles created).
	        //		If an odd number of bytes are sent to WriteSPIPROM function, and it is meant to fill the rest of the page, there will be a byte
	        //		of zero that gets wrapped around to the first byte of the page.
	        if ((bytesToWrite % 2 != 0) && (bytesToWrite == currentPageEnd)){
	            uint8_t firstByteOfThisPage = ReadSPIPROM(PTR-(PTR % PAGE_SIZE)); // Read the first byte of the current page to prevent data loss.

	            uint8_t temp_length = bytesToWrite + 1; 			// Adjust buffer length for an additional byte.
	            uint8_t temp_buffer[temp_length];       			// Declare temp buffer with the new length.
	            memcpy(temp_buffer, rxBuffer, bytesToWrite); 		// Copy received bytes into temp buffer.
	            temp_buffer[temp_length - 1] = firstByteOfThisPage; // Append first byte of next page at the end.

	            WriteSPIPROM(PTR, temp_buffer, temp_length); 		// Write the modified buffer to EEPROM.
	        } else {
	            WriteSPIPROM(PTR, rxBuffer, bytesToWrite); // Write data to EEPROM.
	        }

	        // Adjust bufferIndex and PTR after writing data.
	        if (bufferIndex > bytesToWrite) {
	        	memmove(rxBuffer, rxBuffer + bytesToWrite, bufferIndex - bytesToWrite); // Shift remaining data in buffer to beginning after writing bytes to EEPROM.
	        }
	        bufferIndex -= bytesToWrite;
	        PTR += bytesToWrite;

	        // Wrap PTR if it exceeds the EEPROM's addressable space.
	        if (PTR >= EEPROM_MAX_ADDR) {
	            PTR = 0;
	        }
	    } else if (resp_rdy){
	        // Send response via LPUART when ready.
	        SendLPUARTResp(rec_cnt);
	        resp_rdy = 0;                   // Reset the response ready flag.
	    }
	}
